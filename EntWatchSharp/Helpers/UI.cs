	...
	static class UI
	{
		public static void EWChatActivity(string sMessage, string sColor, Item ItemTest, CCSPlayerController player, Ability AbilityTest = null)
		{
			...
			Server.NextWorldUpdate(() =>
			{
				...
				{
					if (!(pair.Key is { IsValid: true, IsBot: false, IsHLTV: false }) || !EW.g_EWPlayer.ContainsKey(pair.Key)) continue;

					if (Cvar.TeamOnly && pair.Key.TeamNum > 1 && ItemTest.Team != pair.Key.TeamNum && (!AdminManager.PlayerHasPermissions(pair.Key, "@css/ew_chat") || Cvar.AdminChat == 2 || (Cvar.AdminChat == 1 && AbilityTest != null))) continue;
		...
		public static void EWChatAdminBan(string[] sPIF_admin, string[] sPIF_player, string sReason, bool bAction)
		{
			...
			Server.NextWorldUpdate(() =>
			{
				foreach (var pair in EW.g_EWPlayer)
				{
					if (!(pair.Key is { IsValid: true, IsBot: false, IsHLTV: false })) continue;
					using (new WithTemporaryCulture(pair.Key.GetLanguage()))
					{
...
#nullable disable
		...
		public static string[] PlayerInfoFormat(CCSPlayerController player)
		{
			if (player != null)
			{
				...
				sResult[2] = $"{sResult[0]}[{EW.g_Scheme.color_steamid}#{EW.ConvertSteamID64ToSteamID(player.SteamID)}{EW.g_Scheme.color_warning}]";
				sResult[3] = $"{sResult[0]}[{EW.g_Scheme.color_steamid}#{player.UserId}{EW.g_Scheme.color_warning}|{EW.g_Scheme.color_steamid}#{EW.ConvertSteamID64ToSteamID(player.SteamID)}{EW.g_Scheme.color_warning}]";
				return sResult;
			}
